...
Pulls in another \texttt{pconfigure} project that lives in a
subdirectory -- a git submodule, usually.  \texttt{\$dirname} is
relative to the \texttt{Configfile} that names it, and the project it
points at needs no idea that it is being used this way: its
\texttt{Configfile} is read exactly as it would be if you had run
\texttt{pconfigure} inside it.
