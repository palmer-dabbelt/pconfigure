...
gcc -Wall -pedantic -DDEBUG_RETURNS -g `find src/pconfigure/ -iname "*.c"` -L`llvm-config --libdir` -Wl,-R`llvm-config --libdir` -I`llvm-config --includedir` -lclang -lLLVM-2.8 -o "pconfigure" || exit $?
