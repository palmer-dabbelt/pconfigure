...
gcc --std=gnu99 -Wall -DDEBUG_RETURNS -g `find src/pconfigure/ -iname "*.c"` -L`llvm-config --libdir` -Wl,-R`llvm-config --libdir` -I`llvm-config --includedir` `llvm-config --libs core` -lclang `pkg-config talloc --libs` -o "pconfigure" || exit $?
