...
static __inline__ struct talloc_chunk *talloc_chunk_from_ptr(const void *ptr)
{
    ...
    struct talloc_chunk *tc =
        discard_const_p(struct talloc_chunk, pp - TC_HDR_SIZE);
...
*/
_PUBLIC_ void _talloc_set_destructor(const void *ptr,
                                     int (*destructor) (void *))
{
...
/*
  ...
  and in either order
*/
_PUBLIC_ void *_talloc_reference_loc(const void *context, const void *ptr,
                                     const char *location)
{
    struct talloc_chunk *tc;
...
/* 
   ...
   passing NULL as ptr will always return NULL with no side effects.
*/
_PUBLIC_ void *_talloc_steal_loc(const void *new_ctx, const void *ptr,
                                 const char *location)
{
    struct talloc_chunk *tc;
...
*/
_PUBLIC_ void *talloc_reparent(const void *old_parent, const void *new_parent,
                               const void *ptr)
{
    struct talloc_chunk *tc;
    struct talloc_reference_handle *h;
                                       ...
                                       const char *name, const char *expected)
{
    ...
    reason = talloc_asprintf(NULL,
                             ...
                             location, name ? name : "NULL", expected);
    if (!reason) {
...
_PUBLIC_ void *_talloc_get_type_abort(const void *ptr, const char *name,
                                      const char *location)
{
    const char *pname;
...
/*
  ...
  to name the pointer.
*/
_PUBLIC_ void *talloc_named_const(const void *context, size_t size,
                                  const char *name)
{
                                         ...
                                         struct talloc_memlimit *new_limit)
{
    ...
    if (type == TOTAL_MEM_LIMIT &&
        tc->limit != NULL &&
        tc->limit != old_limit && tc->limit->parent == tc) {
...
*/
_PUBLIC_ void talloc_report_depth_cb(const void *ptr, int depth,
                                     int max_depth,
...
static void talloc_report_depth_FILE_helper(const void *ptr, int depth,
...
/*
  report on memory usage by all children of a pointer, giving a full tree view
*/
_PUBLIC_ void talloc_report_depth_file(const void *ptr, int depth,
                                       int max_depth, FILE * f)
{
    if (f) {
        talloc_report_depth_cb(ptr, depth, max_depth,
                               talloc_report_depth_FILE_helper, f);
        fflush(f);
    }
}
...
*/
_PUBLIC_ void *_talloc_zero_array(const void *ctx, size_t el_size,
...
*/
_PUBLIC_ void *_talloc_realloc_array(const void *ctx, void *ptr,
                                     size_t el_size, unsigned count,
                                     const char *name)
{
    if (count >= MAX_TALLOC_SIZE / el_size) {
        return NULL;
...
*/
_PUBLIC_ void *talloc_find_parent_byname(const void *context,
                                         const char *name)
{
    struct talloc_chunk *tc;
